...
class StaticDisplays:
    ...
    def Player_staticDisplay(surface, pos, info):
        ...
        _, _, _, health, angle, hit, r, g, b, active, attack_tick, frames = info
        color = (r, g, b)
    ...
    @staticmethod
    def Bush_staticDisplay(surface, pos, info):
        x, y = pos
        _, _, _, health, angle, hit, r, g, b = info
        darkgreen = (92, 135, 52)
        mutedred = (180, 120, 90)
        base_size = 20
        size = base_size * (health / 15)  # scale down as health decreases
        scale = size / base_size

        def draw_layer(radius, cscale, n):
            points = polygon((x,y), radius*scale-5, n)
            for p in points:
                pygame.draw.circle(surface, darken(darkgreen, cscale), p, 18*scale)

        draw_layer(size, 0.89, 6)
        draw_layer(size, 1.02, 6)
        berries = polygon((x,y), size*0.4+1, 3)
        for p in berries:
            pygame.draw.circle(surface, darken(mutedred, 0.9), p, 9*(scale)**0.8)

    @staticmethod
    def Tree_staticDisplay(surface, pos, info):
        x, y = pos
        _, _, _, health, angle, hit, r, g, b = info
        darkgreen = (92, 135, 52)
        size = 30 * (health / 20)
        p1 = polygon((x,y), size*0.75, 3)
        p2 = polygon((x,y), size*0.75, 3, flip=-1)
        for pset in (p1, p2):
            pygame.draw.polygon(surface, darkgreen, pset, width=13)
            for p in pset:
                pygame.draw.circle(surface, darkgreen, p, 7)
        pygame.draw.polygon(surface, darken(darkgreen, 1.1), polygon((x,y), size*0.75-2, 3))
        pygame.draw.polygon(surface, darken(darkgreen, 1.1), polygon((x,y), size*0.75-2, 3, flip=-1))

    @staticmethod
    def Stone_staticDisplay(surface, pos, info):
        x, y = pos
        _, _, _, health, angle, hit, r, g, b = info
        lightgrey = (130, 130, 130)
        size = 40 * (health / 50)
        pygame.draw.polygon(surface, darken(lightgrey, 0.9), polygon((x,y), size, 8))
        pygame.draw.polygon(surface, lightgrey, polygon((x,y), size-8, 7))
        pygame.draw.polygon(surface, darken(lightgrey, 1.15), polygon((x,y), size/2, 6))
    ...
    def Base_staticDisplay(surface, pos, info):
        ...
        white = (255,255,255)
        size = 40
        pygame.draw.polygon(surface, [lightergrey, white][hit], polygon((x,y), size, 8))
            ...
            match self.active:
                case 1:
                    if self.active_attack == -1:
                        self.active_attack = 1
                        self.startAttack(damage=3, frames=(15,10,7))
                case 2:
                ...
                case 4:
                    if self.active_attack == -1 and self.stone >= self.costs.frag:
                        ...
                        self.startAttack(damage=5, frames=(20,9,8))
                case 5:
...
class Base(StaticObject):
    ...
    def display(self):
        pygame.draw.polygon(self.env.surface, [self.env.colors.lightergrey, self.env.colors.white][self.hit], polygon(self.pos, self.size, 8))
        super().display()
...
class RaiderEnvironment():
    ...
    def step(self, actions=False, display=False):
        ...
        done = (self.base.health <= 0) or (self.t > 10*60*20)
